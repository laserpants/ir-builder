...
spec = describe "LLVM.IRRenderer" $ do
  ...
  describe "renderModule - globals" $ do
    it "renders an extern declaration" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRExtern "printf" (TInt 32) [] False]
              }
      let out = render m
    ...
    it "renders a mutable global variable" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRVar LExternal "sample_tree_cell" TPtr (CNull TPtr)]
              }
      let out = render m
    ...
    it "mutable global uses 'global' not 'constant'" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRVar LExternal "g" TPtr (CNull TPtr)]
              }
      let out = render m
      out `shouldContain` "global"
    ...
    it "mutable global respects linkage" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRVar LPrivate "g" (TInt 32) (CInt 32 0)]
              }
      let out = render m
    ...
    it "mutable global with special-char name renders quoted" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRVar LExternal "my%var" TPtr (CNull TPtr)]
              }
      let out = render m
  ...
  describe "renderModule - floating-point constants" $ do
    it "renders a float constant as a 16-digit hex bit pattern" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRConstant LExternal "f" TFloat (CFloat 0.5)]
              }
      let out = render m
    ...
    it "renders a double constant as a 16-digit hex bit pattern" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRConstant LExternal "d" TDouble (CDouble 0.5)]
              }
      let out = render m
      out `shouldContain` "0x3FE0000000000000"
    ...
    it "renders a non-exact float using the widened double bit pattern" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRConstant LExternal "f" TFloat (CFloat 5.3)]
              }
      let out = render m
  ...
  describe "renderModule - linkage" $ do
    ...
    it "renders private linkage" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRConstant LPrivate "k" (TInt 32) (CInt 32 1)]
              }
      let out = render m
      out `shouldContain` "private"
  ...
  describe "name quoting" $ do
    ...
    it "global with special char renders quoted" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRConstant LPrivate "my%const" (TInt 32) (CInt 32 1)]
              }
      let out = render m
    ...
    it "extern with special char renders quoted" $ do
      let m =
            minimalModule
              { irModuleGlobals = [IRExtern "printf%ext" TVoid [] False]
              }
      let out = render m
  ...
  describe "renderModule - type declarations" $ do
    it "renders a named struct type declaration" $ do
      let m = minimalModule{irModuleTypeDecls = [IRTypeDecl "Node" (TStruct [TInt 32, TPtr, TPtr])]}
      render m `shouldContain` "%Node = type { i32, ptr, ptr }"

    it "renders multiple type declarations" $ do
      let m = minimalModule{irModuleTypeDecls = [IRTypeDecl "Foo" (TStruct [TInt 32]), IRTypeDecl "Bar" (TStruct [TPtr])]}
      let out = render m
    ...
    it "type declarations appear before globals and functions" $ do
      let m =
            minimalModule
              { irModuleTypeDecls = [IRTypeDecl "Node" (TStruct [TInt 32])]
              , irModuleGlobals = [IRExtern "printf" TVoid [TPtr] False]
              }
      let out = render m
  ...
  describe "renderModule - aggregate and vector instructions" $ do
    it "renders extractvalue" $ do
      let instr =
          ...
          m = minimalModule{irModuleFunctions = [IRFunction "f" LExternal (TInt 32) [] [IRBlock "entry" [BlockInstr instr] (IRet (Just (OLocal (TInt 32) "r")))] []]}
    ...
    it "renders insertvalue" $ do
      let instr =
          ...
          m = minimalModule{irModuleFunctions = [IRFunction "f" LExternal (TStruct [TInt 32, TInt 64]) [] [IRBlock "entry" [BlockInstr instr] (IRet (Just (OLocal (TStruct [TInt 32, TInt 64]) "r")))] []]}
    ...
    it "renders insertelement" $ do
      let instr =
          ...
          m = minimalModule{irModuleFunctions = [IRFunction "f" LExternal (TVector 4 (TInt 32)) [] [IRBlock "entry" [BlockInstr instr] (IRet (Just (OLocal (TVector 4 (TInt 32)) "r")))] []]}
  ...
  describe "renderModule - atomic instructions" $ do
    ...
    it "renders cmpxchg" $ do
      let instr =
          ...
          m = minimalModule{irModuleFunctions = [IRFunction "f" LExternal (TStruct [TInt 32, TInt 1]) [] [IRBlock "entry" [BlockInstr instr] (IRet (Just (OLocal (TStruct [TInt 32, TInt 1]) "r")))] []]}
    ...
    it "renders fence" $ do
      let instr =
          ...
          m = minimalModule{irModuleFunctions = [IRFunction "f" LExternal TVoid [] [IRBlock "entry" [BlockInstr instr] IUnreachable] []]}
